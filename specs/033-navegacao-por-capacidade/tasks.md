...
**Goal**: `FR-478` a `FR-483`, `FR-487` e `FR-488`. O **vínculo de comissão** ganha o tratamento que
a capacidade já tem, num ponto único — e as quatro portas que improvisaram passam a usá-lo.
...
- [ ] T021 [US2] Acrescentar a `tests/authorization/test_gramatica_da_recusa.py` os casos de `FR-478` e `FR-479`, **nas quatro portas que erram** — a do marco, a da consulta de Etapa, a da gestão do Processo e a da distribuição: recusa por capacidade e recusa por vínculo respondem recusa explicada, e não "não encontrado"
...
- [ ] T027 [US2] Criar o **ponto único de recusa por vínculo** em `processo_seletivo/seguranca/application/authorization.py`, ao lado de `require_permission` e com a mesma forma: recusa explicada com status 403, e a mensagem nomeando o vínculo. **Mora na camada de segurança, e não em `interface`** (`FR-479`) — foi a ausência desse ponto que produziu quatro improvisos idênticos, e criá-lo em `interface` produziria o quinto
- [ ] T028 [US2] **Separar escopo de vínculo** em `processo_seletivo/interface/views.py::_etapa_para_distribuir`, hoje `if edital is None or pode_gerir_comissao(...) is None: raise Http404` (`FR-488`). São duas condições e duas respostas; enquanto compartilharem um `if`, mudar o status vaza a existência de Editais de outra unidade. **Esta tarefa vem antes de T029**
...
- [ ] T030 [US2] Aplicar o mesmo tratamento ao que o inventário de T003 tiver classificado como recusa de autorização **fora** das seis portas, em `processo_seletivo/interface/views.py`, e **apenas** a isso. Se T004 tiver mudado o escopo, esta tarefa segue a decisão de lá
- [ ] T031 [US2] Enriquecer o `detail` da recusa em `processo_seletivo/interface/views.py::_edital_para_publicar`, hoje *"A operação não é permitida."*, para nomear a capacidade (`FR-481`). O status já está certo desde sempre; o que falta é o motivo
...
- **US2 (Phase 5)**: independente das outras duas em arquivo, deliberadamente última em ordem, e com **ordem interna obrigatória** — T027 (o ponto único) → T028 (separar escopo de vínculo na porta travada) → T029 (as quatro portas passam a consumi-lo). Inverter T028 e T029 responderia recusa explicada para Edital de outra unidade
...
| `seguranca/application/authorization.py` | T027 (US2), sozinha | o ponto único de recusa por vínculo; **nenhuma porta o consome antes de T029** |
