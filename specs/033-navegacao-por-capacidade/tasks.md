...
- [ ] T022 [US2] Acrescentar ao mesmo arquivo os casos de `FR-481` e `FR-489`: a recusa **nomeia as bases que teriam servido** — a permissão de gerir a comissão **e** a presidência daquele Processo, quando as duas serviriam — e declara que nada foi alterado. E o par que prende `FR-489`: **na mesma porta**, uma chamada em que a capacidade de auditoria serve e outra com `somente_gestao=True`, em que não serve, devolvem frases **diferentes**. Se as duas disserem a mesma coisa, o conjunto de bases virou constante e a recusa mente numa delas
