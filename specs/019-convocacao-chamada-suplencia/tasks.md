...
- [X] T001 Criar o app em `backend/processo_seletivo/convocacao/` com `__init__.py`, `apps.py`,
      `domain/__init__.py`, `application/__init__.py` e `migrations/__init__.py`
- [X] T002 Registrar `processo_seletivo.convocacao` em `INSTALLED_APPS` em
      `backend/config/settings/base.py` — o `settings` mora em `config/`, não no pacote de domínio
- [X] T003 [P] Escrever `backend/processo_seletivo/convocacao/domain/nomes.py` com as espécies de
...
- [X] T004 [P] Criar as pastas de teste `backend/tests/unit/convocacao/` e
...
- [X] T005 Estender `apurar` em `backend/processo_seletivo/ocupacao/domain/apuracao.py` para receber
...
- [X] T006 Recusar em `apurar` a determinação de titulares quando houver empate residual não julgado
      atravessando a fronteira do alvo, com o código `empate_na_fronteira_do_alvo` (`R-002`)
- [X] T007 [P] Testar em `backend/tests/unit/ocupacao/test_titulares.py` o cenário da `SC-093`: 40
      vagas, faixa de 70, 67 habilitadas — uma desistência dá 39 e o aceite da suplente devolve 40
- [X] T008 [P] Testar em `backend/tests/unit/ocupacao/test_titulares.py` que **nada muda** onde não
      há desfecho, preservando o comportamento que os testes da `016` já prendem
- [X] T009 [P] Testar em `backend/tests/unit/ocupacao/test_titulares.py` que exclusão de quem não
      era titular não desconta nada, e que duas exclusões da mesma pessoa não produzem `−2`
      (`FR-278d`)
- [X] T010 Atualizar `backend/processo_seletivo/ocupacao/application/emissao.py` para montar a
      sequência ordenada a partir de `PosicaoNaOrdem` e ler os efeitos vigentes do recorte
- [X] T011 Rodar a suíte de `ocupacao` inteira e conferir que nenhum teste da `016` ficou verde
      por acidente com a contagem nova:
      `cd backend && uv run pytest tests/unit/ocupacao tests/integration/ocupacao`

### A porta de efeitos

- [X] T012 Criar `EfeitoDeOcupacao` em `backend/processo_seletivo/ocupacao/models.py` conforme o
[data-model](./data-model.md) §5, com `inscricao_id` e `ato_de_origem_id` como UUID **e não FK**
- [X] T013 Migration da tabela nova em `backend/processo_seletivo/ocupacao/migrations/`, com gatilho
      append-only no mesmo formato das duas tabelas da `016`
- [X] T014 Acrescentar a tabela a `TABELAS_APPEND_ONLY` em
...
- [X] T015 Escrever `backend/processo_seletivo/ocupacao/application/efeitos.py` com
      `registrar_efeito(...)`, transacional, exigindo fundamento e proveniência
- [X] T016 Acrescentar a quinta causa de obsolescência — efeito posterior à apuração vigente — em
      `backend/processo_seletivo/ocupacao/application/selectors.py` e em
      `backend/processo_seletivo/ocupacao/domain/nomes.py`
- [X] T017 [P] Testar em `backend/tests/unit/ocupacao/test_efeito_append_only.py` que `UPDATE` e
      `DELETE` são recusados por gatilho **e** por privilégio ausente
- [X] T018 [P] Testar em `backend/tests/unit/ocupacao/test_obsolescencia.py` que efeito posterior
      torna a apuração vigente obsoleta, com a causa nomeada

### O degrau 15 — a forma de comunicar

- [X] T019 Acrescentar `callForm` ao Perfil publicado em
      `backend/processo_seletivo/publicacoes/application/publish_edital.py`
- [X] T019a [P] Declarar `PUBLICATION` e `INDIVIDUAL_MESSAGE` no vocabulário do conteúdo publicado,
...
- [X] T020 Elevar para `SCHEMA_VERSION = 15` em `backend/processo_seletivo/shared/canonical.py` e
...
- [X] T021 Declarar `"/profiles/*/callForm"` no catálogo de Retificação em
...
- [X] T022 [P] Exibir a forma declarada no documento publicado em
...
- [X] T023 Compor a declaração na interface em `backend/processo_seletivo/interface/forms.py` e no
      passo correspondente do assistente de Edital
- [X] T024 [P] Testar em `backend/tests/unit/publicacoes/test_elevacao.py` que o degrau 15 é
      idempotente e que Edital anterior lê ausência — e **não** um padrão
- [X] T025 [P] Testar em `backend/tests/unit/publicacoes/test_colecoes.py` o endereçamento por
      identidade da nova coleção

### A revisão da `FR-084` da `010`

- [X] T026 Revisar por escrito a `FR-084` em `specs/010-area-do-candidato/spec.md`, registrando a
      redação anterior e admitindo a convocação como terceira situação de mensagem (`R-008`)
- [X] T027 [P] Atualizar `backend/tests/` onde a contagem de situações de e-mail é asseverada, para
      que a regra revisada seja a verificada
- [X] T027a Implementar a guarda `envio_sem_revisao_da_010` em
...
- [X] T028 [P] [US1] Testes de recusa da convocação em
...
- [X] T029 [P] [US1] Testes de append-only em
      `backend/tests/unit/convocacao/test_append_only.py` para `Convocacao` e
      `DesfechoDaConvocacao` — gatilho e privilégio
- [X] T030 [P] [US1] Testes do prazo em `backend/tests/unit/convocacao/test_prazo.py`: vencimento
      informado, recusa de vencimento anterior ao envio, e prazo que não corre sem envio
- [X] T030a [P] [US1] Teste no mesmo arquivo de que o **decurso não produz desfecho** (`FR-274`): a
      leitura mostra o vencimento passado, e nenhum desfecho nasce sem ato de quem conduz
- [X] T031 [P] [US1] Teste de autorização em
      `backend/tests/integration/convocacao/test_autorizacao.py` — negar por padrão, com o papel
      exato de cada ator

### Implementação

- [X] T032 [US1] Criar `Convocacao` em `backend/processo_seletivo/convocacao/models.py` conforme o
      [data-model](./data-model.md) §1, com as três metades de unicidade parcial que o `NULL` do
      `lista_id` obriga
- [X] T033 [US1] Criar `DesfechoDaConvocacao` no mesmo arquivo, com as constraints de forma —
      inércia exige atestado, regularização exige sucessor
- [X] T034 [US1] Criar `ComunicacaoEmitida` no mesmo arquivo, com `enviado_em` anulável e
      `resultado`
- [X] T035 [US1] Migration das três tabelas com gatilho append-only em
      `backend/processo_seletivo/convocacao/migrations/0001_initial.py`
- [X] T036 [US1] Acrescentar as três a `TABELAS_APPEND_ONLY` em
      `backend/processo_seletivo/seguranca/papeis.py` e conferir o `N de M`
- [X] T037 [P] [US1] Escrever `backend/processo_seletivo/convocacao/domain/fila.py` — puro: quem é
      titular, quem é suplente, quem é chamável, e em que ordem
- [X] T038 [P] [US1] Escrever `backend/processo_seletivo/convocacao/domain/prazo.py` — puro: a
      relação entre vencimento informado, envio e decurso
- [X] T039 [US1] Escrever `backend/processo_seletivo/convocacao/application/convocar.py` com as sete
      recusas, autorização, idempotência e auditoria
- [X] T040 [US1] Escrever `backend/processo_seletivo/convocacao/application/desfechar.py` com os
      sete desfechos, um por convocação, chamando `ocupacao.application.efeitos` para excluir ou
      incluir
- [X] T041 [US1] Escrever `backend/processo_seletivo/convocacao/application/comunicar.py` — emissão
      na forma declarada, registro do resultado, e falha que não apaga o ato
- [X] T041a [US1] Compor o conteúdo da mensagem com **o que fazer, o prazo e o canal de
...
- [X] T042 [US1] Escrever `backend/processo_seletivo/convocacao/application/selectors.py` com a
      leitura do recorte e o estado *"convocado, prazo não iniciado"* (`R-009`)
- [X] T043 [US1] Rotas de gestão em `backend/processo_seletivo/interface/views.py`, registradas em
      `backend/processo_seletivo/interface/urls.py`, conforme o
      [contrato](./contracts/convocacao.md) §1
- [X] T044 [US1] Tela `backend/processo_seletivo/interface/templates/interface/convocacao.html` — os
...
- [X] T045 [US1] Confirmação antes de cada ação irreversível, com a mensagem de sucesso nomeando **o
      ato praticado** (`UX-036`) — foi o defeito `E2E16-004` da `016`
- [X] T046 [P] [US1] Testes de interface em `backend/tests/interface/test_convocacao.py`: os estados
      distintos, a confirmação, e a ausência de *"recebido em"*
- [X] T046a [P] [US1] Teste no mesmo arquivo de que a tela **não afirma número de ocupação diferente
      do apurado** (`FR-278a`): entre o desfecho e a emissão seguinte aparece o apurado, com a
      obsolescência declarada
- [X] T047 [P] [US1] Teste de integração em
      `backend/tests/integration/convocacao/test_ciclo_do_77.py` cobrindo a `SC-085` até o desfecho
- [X] T047a [P] [US1] Teste em `backend/tests/integration/convocacao/test_ato_praticado.py` de que
...
- [X] T048 [P] [US2] Teste em `backend/tests/unit/convocacao/test_fila.py` de que a suplência não
      atravessa recorte (`SC-086`)
- [X] T049 [P] [US2] Teste em `backend/tests/unit/convocacao/test_fila.py` do esgotamento da lista
      alcançada, com `lista_alcancada_esgotada`
- [X] T050 [P] [US2] Teste em `backend/tests/unit/convocacao/test_fila.py` de que a Inscrição
      reabilitada por deferimento fica em primeiro e bloqueia quem está abaixo (`FR-292b`,
      `reabilitado_a_frente`)
- [X] T050a [P] [US2] Teste em `backend/tests/integration/convocacao/test_ato_praticado.py` de que
...
- [X] T051 [US2] Implementar em `domain/fila.py` o próximo chamável do recorte, respeitando o teto
      de suplentes que a faixa da `014` alcançou
- [X] T052 [US2] Implementar em `application/convocar.py` a espécie `SUPLENCIA`, recusando
      atravessar recorte
- [X] T053 [US2] Implementar a precedência do reabilitado em `domain/fila.py` e a recusa
      correspondente
- [X] T054 [US2] Mostrar na tela o esgotamento e que a faixa seguinte é ato da `014` — sem oferecer
      botão que esta feature não pode cumprir
- [X] T055 [P] [US2] Teste de integração em
...
- [X] T056 [US3] Acrescentar `REGULARIZACAO` a `ResultadoEtapa.Origem` em
      `backend/processo_seletivo/resultados/models.py`
- [X] T057 [US3] Estender `ck_resultado_origem` com a **quinta** linha legítima — origem nova,
      avaliação nula, anterior presente, fonte jurídica própria (`R-005`) — sem afrouxar as quatro
      existentes
- [X] T058 [US3] Estender o gatilho que confere o Resultado contra a fonte, com o ramo da origem
      nova
- [X] T059 [US3] Migration em `backend/processo_seletivo/resultados/migrations/` — constraint e
      gatilho, nenhuma migration reescrita
- [X] T060 [P] [US3] Teste em `backend/tests/unit/resultados/test_origem_regularizacao.py` de que
      sucessor sem fonte jurídica continua recusado, e de que as quatro linhas antigas seguem
      válidas

### A convocação para regularizar

- [X] T061 [P] [US3] Teste em `backend/tests/unit/convocacao/test_regularizacao.py` de que a
      convocação respeita a ordem de classificação e registra o fundamento do 77 (8.2)
- [X] T062 [US3] Implementar a espécie `PARA_REGULARIZAR` em `application/convocar.py`, admitida só
...
- [X] T063 [US3] Implementar o desfecho de regularização em `application/desfechar.py`: grava o
      Resultado sucessor e **inclui** pela porta, na mesma transação
- [X] T064 [US3] Rota e controle na tela para convocar quem foi indeferido, com o fundamento visível
- [X] T065 [P] [US3] Teste de integração em
...
- [X] T066 [P] [US6] Teste em `backend/tests/interface/test_portal_convocacao.py` de que a tela
      distingue *"não há convocação"* de *"você não foi chamado"*
- [X] T067 [P] [US6] Teste no mesmo arquivo de que a tela escreve *"enviado em"* e nunca
      *"recebido em"* (`UX-039`)
- [X] T068 [US6] Selector da convocação do titular em
      `backend/processo_seletivo/portal/views.py`, restrito à própria inscrição
- [X] T069 [US6] Tela `backend/processo_seletivo/portal/templates/portal/convocacao.html` com a
      chamada, o que fazer, o prazo e o canal de atendimento
- [X] T070 [US6] Registrar o acesso autenticado na trilha **sem** mover o relógio (`FR-288b`)
- [X] T071 [P] [US6] Verificar a tela em 375 px, e não deixar isso para o fim como a `013` deixou
...
- [X] T072 [P] [US4] Teste em `backend/tests/unit/convocacao/test_reclassificacao.py` de que o
      reclassificado passa a constar depois do último suplente
- [X] T073 [P] [US4] Teste no mesmo arquivo da recusa `reclassificado_antes_do_esgotamento`
- [X] T074 [P] [US4] Teste no mesmo arquivo de que a reclassificação **não** afirma perda de
      habilitação (`D-008`)
- [X] T075 [US4] Implementar o desfecho de reclassificação em `application/desfechar.py`: exclui
      pela porta e move na fila
- [X] T076 [US4] Implementar a posição do reclassificado em `domain/fila.py`
- [X] T077 [US4] Mostrar a reclassificação no histórico com o fundamento e a posição nova
...
- [X] T078 [P] [US5] Teste em `backend/tests/unit/convocacao/test_atestado.py` de que inércia sem
      atestado é recusada por constraint, e não apenas por código (`atestado_obrigatorio`)
- [X] T079 [P] [US5] Teste no mesmo arquivo de que o decurso do prazo **não** cancela nada sozinho
- [X] T080 [US5] Criar `AtestadoDeFatoExterno` em `backend/processo_seletivo/convocacao/models.py`
      conforme o [data-model](./data-model.md) §3
- [X] T081 [US5] Migration da tabela em `backend/processo_seletivo/convocacao/migrations/` e entrada
      em `TABELAS_APPEND_ONLY` em `backend/processo_seletivo/seguranca/papeis.py`
- [X] T082 [US5] Escrever `backend/processo_seletivo/convocacao/application/atestar.py` — quem
      atestou, o que concluiu, e a referência do prazo copiada do conteúdo publicado
- [X] T083 [US5] Implementar o desfecho de cancelamento por inércia em `application/desfechar.py`,
      **distinto** do não atendimento à convocação (`D-011`)
- [X] T084 [US5] Rota e controle do atestado conforme o [contrato](./contracts/convocacao.md) §1
- [X] T085 [P] [US5] Teste de integração em
...
- [X] T086 [P] Varredura de vocabulário em
...
- [X] T087 [P] Conferir que `backend/tests/test_vocabulario_da_ocupacao.py` continua verde — a `016`
      segue proibida de falar de convocação, e a fronteira vale nos dois sentidos
- [X] T088 [P] Prova de import em `backend/tests/test_dependencia_da_convocacao.py`: nenhum módulo
      de `ocupacao`, `classificacao` ou `resultados` importa `convocacao`
- [X] T088a [P] Teste no mesmo arquivo de que **nenhum caminho desta feature move quantidade entre
      recortes** (`FR-270`) — mover quantidade é a reversão da `016`, e prova de texto não a cobre
- [X] T088b [P] Medir em `backend/tests/performance/test_convocacao.py` que o histórico responde
      **por conjunto** (`SC-088`): a contagem de consultas não cresce com o número de convocações do
      recorte
- [X] T089 [P] Medir a `SC-090` em `backend/tests/performance/test_convocacao.py` — 1.000
      participantes, teto de 3 s, e contagem de consultas que **não** cresce com o número de
      convocações
- [X] T090 [P] Histórico com proveniência em
      `backend/processo_seletivo/interface/templates/interface/convocacao_historico.html` (`FR-294`)
- [X] T091 [P] Trilha de auditoria do Edital listando os atos desta feature em linguagem humana
      (`FR-296`)
- [X] T092 Matriz de rastreabilidade em `specs/019-convocacao-chamada-suplencia/rastreabilidade.md`
      — `FR-264`–`FR-296`, `SC-085`–`SC-094`, `UX-035`–`UX-039`, cada um com o teste que o prende
- [X] T093 Percurso conduzido pela interface seguindo o [quickstart](./quickstart.md), com relatório
      em `doc/e2e/019-convocacao/relatorio.md`
- [X] T094 Atualizar a tabela de incrementos do `README.md` com a `019` e a contagem nova da suíte,
      e o `N de M` do provisionamento em `README.md` e `AGENTS.md` — o resíduo que reapareceu cinco
      vezes
- [X] T095 `cd backend && make lint check test-pg` verde, e `uv run pytest
