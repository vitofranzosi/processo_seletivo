...
| **FR-737** | `editais/domain/cronograma.validate_event` (`STATUS_DECLARAVEIS`); a tela normaliza o herdado em `forms.eventos_persistidos` | `test_a_fase_ordinaria_nao_se_declara` (dois valores), `test_o_contrato_do_evento_so_anuncia_os_estados_que_aceita`, `test_regravar_normaliza_a_fase_que_a_api_gravava_antes_da_045`, `test_regravar_o_cronograma_preserva_o_que_a_tela_nao_oferece` |
...
| **FR-742** | `resumo_da_etapa` conta participantes — pelo panorama ou por `_so_participantes`; a Supervisão passa o conteúdo; filtro `carente` e ficha *"Participantes desta Etapa"* na distribuição | `test_a_cobertura_conta_so_os_participantes_e_as_duas_formas_concordam`, `test_com_o_conteudo_na_mao_a_cobertura_nao_rele_a_versao_vigente`, `test_o_numero_sem_avaliador_suficiente_abre_uma_lista_do_mesmo_tamanho`, `test_inscricao_sem_avaliador_e_carente_e_permanece_no_denominador` (022), `test_o_custo_nao_cresce_com_a_populacao` e `test_a_primeira_etapa_nao_consulta_progressao` (teto intacto) |
...
| **UX-086** | a mensagem de `_etapa_sem_evento`, que **nomeia** a Etapa — a API a devolve crua —; *"Aviso"* na lista de pendências | `test_a_etapa_sem_evento_e_aviso_na_publicacao` (nome na mensagem, sem caminho interno, sem *atraso*, *aguarda*, *pendente*), `test_a_etapa_sem_evento_e_aviso_na_etapa_e_na_revisao` |
...
## 8. Revisão do PR, 26/09/2026

Três achados, os três procedentes e corrigidos no próprio PR:

| Achado | Correção | O que o prende |
|---|---|---|
| o resumo passava o conteúdo e não as Etapas, e a restrição relia a versão vigente por Etapa — duas consultas, e a mistura de versões sob uma Retificação concorrente | `resumo_da_etapa` monta `vigentes` do mesmo conteúdo e o repassa | `test_com_o_conteudo_na_mao_a_cobertura_nao_rele_a_versao_vigente` — vermelho sem a correção, conferido |
| o serializer anunciava as quatro fases e a API recusava duas | o `ChoiceField` oferece `PLANEJADO` e `CANCELADO`, e a escolha inválida responde com a razão do domínio | `test_o_contrato_do_evento_so_anuncia_os_estados_que_aceita`, `test_a_fase_ordinaria_nao_se_declara` |
| o aviso chegava à API sem o nome da Etapa, só com o caminho interno | a mensagem nomeia a Etapa; o caminho fica no `path` | `test_a_etapa_sem_evento_e_aviso_na_publicacao` |
