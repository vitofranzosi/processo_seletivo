...
3. **`T042` é lida CASO A CASO, e nunca pela contagem.** Um teste pode manter o número de asserções e
...
- [ ] T021 [US1] Decidir e implementar o recorte **sem nenhum autodeclarado**, conforme a tabela de `research.md` `R-8`: ordem vazia é **emitível e nunca automática**, e a tela do recorte vazio diz que ninguém concorreu ali — em vez de parecer pendência. Toca `backend/processo_seletivo/classificacao/application/emissao.py` e `backend/processo_seletivo/interface/templates/interface/ordenacao.html` (**EXISTENTE**), e a decisão vai para `specs/034-ordem-por-recorte/research.md` (**EXISTENTE**, acrescente ao fim)
- [ ] T022 [US1] Decidir e registrar os **dois** casos de borda que a spec nomeia e nenhuma tarefa fechava, em `specs/034-ordem-por-recorte/research.md` (**EXISTENTE**, acrescente ao fim), prendendo cada decisão por teste em `backend/tests/integration/classificacao/test_ordem_por_recorte.py` (**EXISTENTE**, acrescente): (a) **Retificação que acrescenta Modalidade depois de a ordem da ampla já ter sido emitida** — o recorte novo nasce sem ordem, e a da ampla fica vigente ou obsoleta? (b) **empate residual que atravessa a fronteira do alvo em dois recortes ao mesmo tempo** — o julgamento vale para as duas ordens. Se qualquer das duas exigir mudança de escopo, **pare e pergunte**
...
- [ ] T032 [US3] Mudar a **resposta** de `backend/processo_seletivo/editais/domain/marcos.py::emite_ordem_no_recorte` (**EXISTENTE**) — `FR-500`. Os consumidores são **dois**, contados em `research.md` `R-4`, e os dois seguem daqui. **Nenhuma segunda pergunta é criada**: um predicado próprio na classificação seria exatamente o defeito que a `032` registrou como razão de a função existir
- [ ] T033 [US3] Decidir e aplicar o destino do aviso `reserved_row_without_ordering` em `backend/processo_seletivo/editais/domain/validation.py` (**EXISTENTE**): **aposentar** ou **estreitar**, e dizer qual **na `spec.md`** (**EXISTENTE**) — é a própria `FR-501` que exige isso, e não o `research.md`: *"o que sobrar dele MUST ser dito na spec"*. Registre também o caminho da decisão em `specs/034-ordem-por-recorte/research.md` (**EXISTENTE**, acrescente ao fim). A escolha muda a conta de T035, e é por isso que é tarefa e não detalhe
- [ ] T034 [US3] Retirar de `backend/processo_seletivo/interface/templates/interface/ocupacao.html` (**EXISTENTE**) a frase que manda apurar fora do sistema, onde a apuração passou a existir (`FR-502`). Ela deixou de ser verdade, e frase verdadeira que virou falsa é pior do que frase ausente
- [ ] T035 [US3] Atualizar os **cinco** casos de `backend/tests/unit/editais/test_executabilidade.py` (**EXISTENTE** — altere **só** os cinco, nomeados em `research.md` `R-5`). Os outros cinco **permanecem**, e um deles — `…a_modalidade_declarada_como_ampla_nao_e_lida_como_reserva` — passa a ser a contraprova da derivação única
- [ ] T036 [US3] Atualizar os **dois** casos de `apuravel` em `backend/tests/interface/test_ocupacao.py` (**EXISTENTE**) e o caso do hardening em `backend/tests/interface/test_hardening_pos_auditoria.py` (**EXISTENTE**). **Os três casos vizinhos, da família do corte (`FR-463`), NÃO mudam** — estão nomeados porque a semelhança convida ao erro
- [ ] T037 [US3] Corrigir, em `backend/tests/integration/ocupacao/test_reversao.py` (**EXISTENTE**), a frase que justifica o helper construído à mão: *"o caminho do ator não a alcança em certame computado"* deixou de ser verdade. **Não apague o helper** — ele é chamado por seis casos, e trocá-lo mudaria o que eles exercitam
...
- [ ] T038 Percorrer o **cenário 1** de `specs/034-ordem-por-recorte/quickstart.md` (**EXISTENTE**) pela interface administrativa, com o seletor de identidade ligado — inclusive a contraprova do Edital sem reserva —, e registrar o observado para a T044
...
- [ ] T040 [P] Percorrer o **cenário 3** de `specs/034-ordem-por-recorte/quickstart.md` (**EXISTENTE**) — a Revisão sem o aviso, a contraprova do marco que sorteia, e o Edital do acervo intocado
- [ ] T041 [P] Percorrer o **cenário 4** de `specs/034-ordem-por-recorte/quickstart.md` (**EXISTENTE**) — o acervo antes e depois, idênticos, e o censo dos degraus de elevação sem degrau novo (`FR-504`, `SC-173`)
- [ ] T042 Conferir **caso a caso** os onze testes alterados contra o "antes" gravado em `specs/034-ordem-por-recorte/antes-da-ordem-por-recorte.md` (**EXISTENTE**, criado em T002), e registrar ali mesmo a comparação. **Nunca pela contagem**: um caso pode manter o número de asserções e trocar o que afirma. Alargar um conjunto aceito é enfraquecer a asserção, e a suíte fica verde do mesmo jeito
- [ ] T043 Varrer os **doze Editais da amostra real** de `doc/avaliacao-de-capacidade-editais-2026-09-12.md` (**EXISTENTE**, leitura) e registrar, em `specs/034-ordem-por-recorte/varredura-da-amostra.md` (**NOVO**), quais avisos da família da `032` ainda disparam em cada um e quais deixaram de disparar (`SC-174`). A `032` fez essa varredura e foi ela que confirmou a decisão de tratar por aviso — checklist, `analyze` e o teste de citações ficam verdes com regras que se contradizem, e só a leitura Edital a Edital não fica
- [ ] T044 Escrever `specs/034-ordem-por-recorte/rastreabilidade.md` (**NOVO**), citando a varredura de T043: uma linha por `FR-`, uma por `SC-` — e **uma por teste alterado, com o motivo**. É o que separa fechar o `ACH-47` de afrouxar a ordem
- [ ] T045 Rodar `cd backend && make lint check test-pg` e registrar a contagem final em `specs/034-ordem-por-recorte/rastreabilidade.md` (**EXISTENTE**, criado em T044). `test-pg` e **nunca** `test`; `lint` são **dois** passos. **Não edite arquivo do projeto enquanto a suíte roda**. O `make check` inclui `makemigrations --check`, e **é ele o guarda da `FR-505` e da `SC-175`** — registre o resultado dele com esse nome, porque "nenhuma migration" é promessa que só se prova rodando
...
- **T032 → T033** — o destino do aviso depende de o predicado já ter mudado de resposta.
- **T033 → T035** — a escolha entre aposentar e estreitar muda quais casos mudam.
...
| T040 · T041 | percursos independentes, um sem banco e outro só de leitura |
