...
- [X] T129 Estender as varreduras ao portal e às **combinações** de classe, e desfazer o reformato acidental de 33 arquivos da 010: `.marco.concluido` passa a existir (dos três estados da linha do tempo, dois tinham desenho), `.papel`, `.explica` e o `summary` dos documentos anunciados ganham o seu, e o separador do cartão da vitrine deixa de aparecer sozinho quando o Processo não tem código publicado
- [X] T128 Polir a lista de Processos em `lista.html` e `base.html`: `.acao.desabilitado` passa a existir (a ação indisponível saía com a borda verde de quem convida), a contagem diz que conta Editais, o cabeçalho vira dois grupos com o estado junto do nome e as ações à direita, "Cancelar" e "Encerrar" ganham a advertência que `detalhe.html` já dava, as colunas passam a alinhar entre cartões, e o estilo de linha sai do template
