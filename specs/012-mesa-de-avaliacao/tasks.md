...
- [X] T084 Acrescentar o parâmetro opcional `actor_subject` a `consultar()` em `backend/processo_seletivo/auditoria/selectors.py` — assinatura, não esquema, como `record_event` ganhou dois na 011 — e criar `trilha_da_avaliacao()` no mesmo arquivo, com a **consulta composta** de T-016: por inscrição, os agregados relacionados (`Inscricao`, `Atribuicao`, `Avaliacao`, `Impedimento` daquela inscrição); por avaliador, os relacionados daquela identidade estável **mais** os eventos de `Inscricao` restritos a `actor_subject` igual a ela, porque abrir documento é o único ato cujo agregado não distingue avaliadores; combinado, a interseção
