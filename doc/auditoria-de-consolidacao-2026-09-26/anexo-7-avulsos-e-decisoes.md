...
| 21 | **ValorDeFato: gatilho depois da 044** · 25/09 · memória `valor-de-fato-corrigir-depois-da-044.md`; PR #171 (aberto, `doc/achado-valor-de-fato-sem-gatilho.md`) | `inscricoes_valordefato` ganha gatilho `BEFORE UPDATE OR DELETE` e guarda de modelo como `inscricoes/0006`, **só depois** da migration `0005` da 044 estar na main; escopo só `ValorDeFato` | **Adia** a correção; **recusa** incluir `PosicaoNaOrdem`, `RevisaoEdital`, `GeracaoDeArquivo` (só registradas) | Hoje só a camada de privilégio: `seguranca/papeis.py:39`; `inscricoes/migrations/` vai até `0004_valor_de_fato.py` na main | **Sim, e bloqueado**: a precondição ("044 mergeada") **não** está satisfeita — o #168 mesclou só a spec; `0005_item_da_lista_exigida.py` existe só na branch da 044. Ver Incertezas |
...
Decisões (Parte 2): 25 registradas. Com trabalho criado **não executado**: D-G1, D-G2, D-G3 (regra do reuso), D-G5, recorte documental (merge da 044 + "O que mudou" do recorte + opção 2), ValorDeFato (bloqueado), D-4 (barema adiado), nome social (029), TF-1/TF-2 (043), pergunta 3 da presidência única, papel de Diretoria (040).
...
- **NOVO-2 — a 044 está implementada, mas fora da main e sem PR.** `origin/claude/044-recorte-transversal-documental` tem 8 commits (`bccfda0` → `56649d4`, este de 25/09 23:28, cinco minutos depois do merge do #168 às 23:23) com a implementação completa (migration `inscricoes/0005_item_da_lista_exigida.py`, `modalityCode`, lista gravada no envio, Retificação). O #168 mesclou só spec/plano/tarefas, e a lista de PRs abertos tem só #171 e #172. Consequência: a correção do `ValorDeFato` (memória de 25/09) espera "o merge da 044 (#168)" — a condição literal (#168 mesclado) está satisfeita, mas a real (migration `0005` na main) não. O briefing deste lote ("a 044 está só especificada") vale só para a main.
...
2. **A 044 da branch vai para a main, e por qual PR?** Isso destrava a correção do `ValorDeFato` e a D4 do recorte documental. Confirmar se a memória "depois do merge do #168" quis dizer a implementação.
