...
**E2E15‑014 · UX · portal → REDAÇÃO CONTESTADA · "Minhas inscrições" não mostra o rascunho** — após "Seus dados foram guardados… Você pode continuar a inscrição que começou", a lista diz "Você ainda não possui inscrições"; retomar exige voltar ao Edital e clicar "Inscrever‑se" de novo. Evidência: `24-candidata-apos-login.png`, `28-inscricao-modalidade.png`.
- **Reverificação (06/09/2026):** a causa afirmada não procede. `portal/views.py::inscricoes` consulta `Inscricao.objects.filter(identity_subject=…)` **sem filtro de situação**, e `inscricoes.html` desenha o rascunho explicitamente — "Inscrição não enviada" e ação "Continuar inscrição". Três testes já afirmam isso (`tests/integration/portal/test_minhas_inscricoes.py`, `test_convite_por_vaga.py`, `test_recusas_de_abertura.py`). As próprias evidências citadas apontam a explicação: `24-candidata-apos-login.png` é **anterior** a `28-inscricao-modalidade.png` — a lista foi lida no minuto seguinte ao login, antes de o rascunho existir, e ali "Você ainda não possui inscrições" é o estado vazio correto.
- **Redação proposta:** *"E2E15‑014 · UX · portal — O convite pós‑login não distingue 'ainda não comecei' de 'comecei e não terminei'"* — o que o auditor viu foi a lista vazia logo após o acesso, e o que a interface não faz é dizer, na mensagem de retomada, que o rascunho só passa a existir depois de escolher a vaga. Se essa for a formulação certa, o achado é **P3 de texto**, não de consulta. **Não corrigido:** a nova redação é proposta, e a decisão de reclassificar ou arquivar o achado é de quem conduziu a auditoria.
...
| P3 | 8 | 009, 010, 011 — **corrigidos** em `claude/e2e-015-audit-readability-288630` · 012, 013, 016 abertos · 014 com redação contestada · 015 é registro de desenho |
| — | 1 | 002 (retratado — não era defeito) |

Abertos ao fim desta rodada: **003** (depende de decisão de governança), **007**, **012**, **013**, **016**; **014** aguarda decisão sobre a redação proposta.
...
6. ~~**Legibilidade da 015**~~ — **feita** em `claude/e2e-015-audit-readability-288630`: as telas de ato/proveniência trazem nomes ao lado dos identificadores, datas em português e o diff que mostra o que mudou. Resta o **rótulo publicado dos critérios de desempate** na tabela do ato, que hoje sai como enum (`MAIOR_VALOR_DE_FATO`): a proveniência congelada guarda o `type`, e não o rótulo, então resolvê‑la exige decidir de qual versão vem o nome do fato — mesmo raciocínio que esta rodada aplicou às modalidades.
...
- ~~Nomes legíveis nas telas da 015 (E2E15‑006) e datas localizadas (E2E15‑009)~~ — **corrigidos** em `claude/e2e-015-audit-readability-288630`, junto com o diff legível (E2E15‑011) e o aviso de sucessão (E2E15‑010). Resta ali o rótulo publicado dos critérios de desempate na tabela do ato.
