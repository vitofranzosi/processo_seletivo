...
|---|---|---|---|
| 1 | **divergência real entre duas avaliações da mesma inscrição** | 🔴 **aberto, e o beco não é o que este relatório supõe** | Virou achado próprio: [a Etapa que prevê duas avaliações publica um ato que não se consolida](achado-duas-avaliacoes-sem-regra-de-combinacao.md). Em resumo: `evaluationsPerRegistration` é campo de composição **sem teto e sem aviso**; a `032` não o conhece; a publicação — **ato imutável** — aceita; a distribuição atribui as duas corretamente; e a consolidação recusa a **Etapa inteira**, com razão, porque *"o Edital não declara como combiná-las"* — e **não existe campo para declarar**. A recusa está certa; o defeito é o momento |
